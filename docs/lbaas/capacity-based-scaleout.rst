...
To find the capacity score, the |agent| divides the collected metric by the max specified for that metric in the :code:`capacity_policy` `configuration parameter </products/openstack/latest/agent/index.html#configuration-parameters>`_.
